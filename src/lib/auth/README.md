...
- **GET `/api/role?address=xxx`** - Lấy roles của user từ blockchain
- **GET `/api/reputation?address=xxx`** - Lấy reputation score
- **GET `/api/job`** - Lấy danh sách jobs
- **GET `/api/job/[id]`** - Lấy chi tiết job
- **GET `/api/ipfs/get?cid=xxx`** - Lấy metadata từ IPFS
- **GET `/api/chat/messages?roomId=xxx`** - Lấy messages (read-only)
